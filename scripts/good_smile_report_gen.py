...
<div class="wrap">
  ...
  <section class="block">
    ...
    <div class="insight">
      ...
      <p>4→7月は20→9と改善傾向だったが、8月に33へ急悪化(整骨院・整体・肩こりの3KWが揃って悪化)。8月単月の変動である可能性も残るため、9月以降の継続性を要確認とする。HP経由の急伸(+132.7%)は、広告出稿がHP経由に合算されている可能性が高く、内訳を分離できていない現時点では評価保留とする。</p>
      <h4>原因仮説(推測・要検証)</h4>
      <ul>
        ...
        <li>HP経由急伸: 広告効果の可能性が高いが、Google PPC広告・META広告の実績シート未取得のため、CVR・CPAが伴っているか(UUの無駄喰いでないか)は未検証</li>
      </ul>
      <h4>次のアクション(優先順位順)</h4>
      <ol>
        <li>HPB掲載5院の予約枠(K/L)稼働状況を今月中に確認(ACR急落の一次仮説検証)</li>
        <li>HPB口コミ数・評点の推移データ取得の運用立ち上げ</li>
        <li>Google PPC広告・META広告の実績シートへのアクセス権を確認・共有依頼(HP経由急伸の内訳分離)</li>
        <li>MEO順位悪化(整骨院・整体・肩こり)が9月以降も継続するか追跡</li>
        <li>自然検索CVRの8月分取得、GA4でSEO/MEO流入の実UU/CVRと順位変動の相関を確認</li>
      </ol>
      <div class="data-gap"><b>まだ手元にないデータ(確認済みの事実)</b>: HPB口コミ数・評点の推移、予約枠(K/L)稼働状況、Google PPC広告・META広告の実績、自然検索CVR(8月分)、GA4のSEO/MEO流入UU/CVRはいずれも今回未取得。推測値では埋めていない。</div>
    </div>
  </section>
