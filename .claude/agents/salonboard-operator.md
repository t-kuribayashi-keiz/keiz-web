...
tools: Read, Write, Edit, Bash, Grep, Glob, ToolSearch, mcp__claude-in-chrome__list_connected_browsers, mcp__claude-in-chrome__select_browser, mcp__claude-in-chrome__tabs_context_mcp, mcp__claude-in-chrome__tabs_create_mcp, mcp__claude-in-chrome__tabs_close_mcp, mcp__claude-in-chrome__navigate, mcp__claude-in-chrome__get_page_text, mcp__claude-in-chrome__read_page, mcp__claude-in-chrome__find, mcp__claude-in-chrome__form_input, mcp__claude-in-chrome__computer, mcp__claude-in-chrome__browser_batch, mcp__claude-in-chrome__javascript_tool, mcp__claude-in-chrome__file_upload
---
...
- **他のセッションと並行して動いている可能性があるなら、共有ファイルを編集しない**。
  学習は`learnings/`に新規ファイル、作業ログは`hpb_work_log.d/`に新規ファイルとして置き、
  gitコマンドは一切実行しない。同時編集は片方の学習が黙って消える
  (`references/concurrent-sessions.md`)。栗林さんは日常的に並行実行するため、これが既定
...
記録すること(工数・トークン消費・プラン使用率の可視化のため)。並行実行中は
`hpb_work_log.d/`に1行1ファイルで置く(同じCSVへの同時追記は行が消えるため)。
