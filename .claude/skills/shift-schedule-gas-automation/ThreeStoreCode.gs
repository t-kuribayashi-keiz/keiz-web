 ...
 *
 * 機能（カッコ内はメニュー上の場所。初期設定は最初の1回だけ、日常運用は毎月行う操作）:
 *   ・スタッフマスターの雛形を作成（初期設定・メンテナンス）
 *     名前/性別/資格/新患対応/院長/ピラティス/店舗を管理する別タブ
 *   ・原本テンプレートを作成（初期設定・メンテナンス／初回のみ）
 *     日付・曜日・店舗ごとの色分けブロックを1から自動生成します。日付の下には、院長会議・
 *     AM2.3年目研修など店舗をまたぐ全社/エリア共通の定例予定を書き込める「行事欄」も3店舗共通で
 *     1行分用意します。希望休/公休の自動入力・自動クリアの対象外です
 *   ① 月次シートを作成（メニュー本体／日常運用）
 *     「原本」テンプレートをコピーし、日付を自動配置。日本の祝日は日付の文字を赤にします
 *   ② 公休を自動入力（メニュー本体／日常運用）
 ...
 *   5. 初回のみ、メニュー内のサブメニュー「初期設定・メンテナンス」から
 *      「スタッフマスターの雛形を作成」→「原本テンプレートを作成」の順に実行し、
 *      スタッフマスターに全スタッフを登録してください
 *   6. 準備ができたら、メニュー本体の「① 月次シートを作成」→「② 公休を自動入力」の順に進めてください
 *      （日常の運用はこの2つだけで完結します）
 *   7. 初回実行時は権限の承認が必要です（祝日取得は外部URL取得の承認が必要です）
 */
...
function createThreeStoreTemplateSheet() {
  ...
  DATE_ROWS.forEach((dateRow) => {
    // 日付行：曜日ごとに1セル、太字・中央寄せにしておく（実際の日付は「① 月次シートを作成」で入る）。
...
function createMonthlySheetCore(ss, year, month, quotas) {
  ...
  if (!srcSheet) {
    return {
      ok: false,
      message: `シート「${TEMPLATE_SHEET_NAME}」が見つかりませんでした。先に「初期設定・メンテナンス」→「原本テンプレートを作成」を実行してください。`,
    };
  }
...
function autoFillRegularHolidaysCore(sheet, quotas) {
  ...
  if (masterRaw === null) {
    return {
      writeCount: 0,
      message: `「${MASTER_SHEET_NAME}」シートが見つかりません。先に「初期設定・メンテナンス」→「スタッフマスターの雛形を作成」から作成してください。`,
    };
  }
