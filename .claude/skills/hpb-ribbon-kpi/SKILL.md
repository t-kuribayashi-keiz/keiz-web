...
6. **作業を始める前に、必要なものが揃っているか確認してから進める。** 「リボンデータ反映して」
   とだけ言われて何も準備が無い状態で来ることがある(例: チャット上のショートカットから
   コピーしたフレーズをそのまま送っただけ等)。以下が揃っていなければ、憶測で進めず
   その場で聞き返す:
   - **ローカルのClaude Codeセッションか**(クラウド実行環境ではPDFファイルに触れないため、
     抽出ステップ自体が不可能。クラウドで来たら「ローカルのセッションで実行してください」
     と伝えて止まる)
   - **対象月号**(「今月分」は曖昧。リボンは基本的に直近の完了月号を指すことが多いが、
     具体的に「2026年◯月号」の形で確認する)
   - **リボンZIPが展開済みか・パスワードは手元にあるか**(パスワードは絶対にチャットに
     書かせない。`HPB_RIBBON_PASSWORDS`環境変数として栗林さんの手元で設定してもらう)
   - **対象ブランド(`--profile`)**: 既定は`chokuei`(直営+サンズミライ)。スマイル・グッドを
     指す場合は`smile-good`と明示してもらう
   - **書き込み先の準備が済んでいるか**: [functions/hpb-ribbon-kpi/CLAUDE.md](../../../functions/hpb-ribbon-kpi/CLAUDE.md)
     の「セットアップ(未完了)」表を必ず確認する。`chokuei`プロファイルは2026-09-10時点で
     **Masterシートの書き込み用サービスアカウントへの共有がまだ「未」**(=`apply`が失敗する)。
     未了の項目がある場合は、`apply`を試みる前にその項目を栗林さんに伝え、対応してもらって
     から進める
