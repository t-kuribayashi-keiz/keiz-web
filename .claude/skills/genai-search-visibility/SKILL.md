...
- **2026-05〜08月分(147店舗)**: 2026-09-15に全店舗収集完了。結果は
  `data/genai-search-visibility-results/2026-05to08_batch1.csv`(147行)。
  レポートArtifact: https://claude.ai/artifact/Ek9uaW34vziFtTYu74nmJi
  - **除外基準(栗林さんと合意)**: 4か月とも表示回数が全て2桁以下(0〜99)、または
    全月データが生成されなかった店舗は、ベータ版の計測不具合の可能性が高いとして
    集計・グラフから除外する。この基準で147店舗中42店舗を除外し、有効105店舗で集計。
    **除外42店舗が「計測不具合」なのか「実際にAI Overviews露出がほぼ無いだけ」なのかは
    未検証** — 翌月以降の収集でも同じ店舗が引き続き2桁以下なら、不具合ではなく実態と判断する。
  - 5月は`references/procedure.md`4節の注記どおり14日分(5/18〜5/31)換算である点に注意。
  - 次回以降の月次収集は、このCSVをベースに新しい月の列を追加していく運用を想定。
