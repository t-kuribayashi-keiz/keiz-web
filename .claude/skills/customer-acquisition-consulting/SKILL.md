...
The *originating* engagement (T&D Group, 8-store 整骨院 chain, `td-group.jp`) never got past the Google Cloud/service-account setup before stalling on a client-side permission grant — see `references/background.md` for exactly how far it got. **But the same architecture has since gone into real production for this repo's own clients**: GA4/GSC自動取得(`analytics-sync`、`skills/README.md`参照)がスマイル・グッドで2026-09から月次cronとして稼働中、GRCのCSV自動化も2026-09-18に稼働開始している。So treat the T&D background as "how this stalled once," not as the current state of the pattern — check `skills/README.md` and `docs/org-review-log.md`'s 2026-09-19 entry for what's actually live before assuming you're starting from zero.
