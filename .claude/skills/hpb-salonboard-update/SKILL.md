...
0. **This skill only works in a local Claude Code session with `claude-in-chrome` access to the user's real, logged-in Chrome.** A cloud/hosted execution environment has no browser to drive and cannot reach salonboard.com's admin UI at all. If this trigger arrives in a session without `mcp__claude-in-chrome__*` tools available (check via `ToolSearch` if unsure — see the "Load the MCP tool schemas first" note below), **stop immediately and tell the user to run this from their local PC instead of a cloud session** — do not attempt to browse, guess at an API, or otherwise improvise a workaround.
