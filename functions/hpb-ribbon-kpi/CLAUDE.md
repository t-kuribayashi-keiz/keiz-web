...
## Master右側の拡張列(T列〜、2026-09-04 追加)

栗林さんの指示: **既存A〜S(19列)の列構成は変えず、右=T列以降にリボンの拡張項目を追記**する。
過去月の行はT列以降が空欄のまま。ヘッダー(1行目)のT列〜に列名を置く。GASはA〜S列を固定
インデックスで読むだけ&Masterへは書き込まないため、右appendの影響を受けない。

拡張28項目は [data/hpb-ribbon-config.json](../../data/hpb-ribbon-config.json) の
`master_sheet.master_ext_columns`(= `hpb_ribbon_extract.EXTENDED_COLS` と一致):
口コミ数/評点(総合+内訳4)/返信率/月号新規口コミ数/直近平均点/ブログ数/月次投稿数/残キャパ50以上/
設備数/クーポン数/最寄駅/男性率/ALL予約数/リピート予約数/売上万円/客単価/マイページ登録数/
比較サロン平均のPV・CVR・ACR・口コミ数・評点・返信率。writerは`--apply`時に、拡張ヘッダーが
未設定ならT1に列名を入れてから、本体を A〜AU(47列)で書き、全幅で読み返して照合する。

**P3(掲載状況)の「-」許容(重要な既知の罠)**: 新規/小規模店は自サロン行の設備数などが「-」に
なる。数値と同格に「-」を許容しないと、正規表現が自サロン行をスキップして**比較サロン一覧の
先頭店の数値を拾う**(例: 本八幡南口鍼灸の口コミ数に、比較サロンの942が入る)。`_VAL`パターンで
「-」を許容して防ぐ。tests/test_hpb_ribbon_extract.py `test_p3_dash_does_not_grab_competitor`。
...
なるため、**本体と鍼灸の両方がMasterにある**ときだけ、素の名前が集客数側に近い方(本体)に値を入れ、
もう一方は**空欄**にする(二重計上を防ぐ)。writerが自動でこの按分を行い、`[併設按分]` として
ログに出す。**Masterにその拠点の鍼灸行しか無い場合は按分せず、その1行に値を入れる**
(本体行が無く二重計上の心配がないため)。2026年8月号の按分は5件(八幡宿駅西口/新静岡駅前/
本八幡南口/新潟関屋/ライフガーデン茂原)。郡山若葉町・豊四季北口は鍼灸行しか無いのでその行に計上。
