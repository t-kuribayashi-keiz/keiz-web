...
| `smile-good` | スマイル11院+グッド7院 | グッド・スマイル月次報告 の『◯月HP(速報値)』タブ内の**HPBブロック** | `GCP_SMILE_GOOD_KEY` | **未決定(null)** |
...
### 転記先が未決なので、いまは inspect だけ

`smile-good` の `master_sheet` は `null`。決めるべきは
「直営の145店舗Masterに18院を足す」か「スマイル・グッド専用のMasterを作る」かで、
これは栗林さんの判断(前者は既存のGASダッシュボードの集計母数が変わる)。null のあいだ
writer は `--mode inspect` 以外を受け付けない。inspect は集客数の結合(名寄せ・未マッチ)
だけを確かめるので、鍵さえ入れば今すぐ答え合わせできる。
...
| # | 対象 | 権限 | 誰が | 状態 |
|---|---|---|---|---|
| 1 | `GCP_SMILE_GOOD_KEY` をリポジトリSecretに登録(鍵はローカルの `claude\keys\smile-good-reporter.json`) | — | 栗林さん | 未 |
| 2 | スマイル5院・グッド7院ぶんのリボンPDFがHPBから届いているかの確認 | — | 栗林さん | **未確認**(届いていなければ、そもそも抽出する元が無い) |
| 3 | 転記先Masterの決定(145店舗Masterに足す/専用Masterを作る) | — | 栗林さん | 未 |
| 4 | 転記先に `smile-good-reporter` を**編集者**で共有(現状は閲覧者) | 編集者 | 栗林さん | 未 |

2の答えが「届いていない」なら、埋められるのは予約枠(×の数)だけになる。この場合は
`hpb-reservation-slot-check` 側に18院を足す話になり、本functionの範囲外。
